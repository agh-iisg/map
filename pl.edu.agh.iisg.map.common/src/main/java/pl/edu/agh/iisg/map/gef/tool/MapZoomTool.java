...
public class MapZoomTool extends TargetingTool implements MouseWheelHelper {
    ...
    public void mouseWheelScrolled(Event event, EditPartViewer viewer) {
        ...
        Point diff = new Point(event.x - mapDimesion.width / 2, event.y - mapDimesion.height / 2);

        double oldZoom = zoomLevel;

        Point oldPoint = new Point(driver.longitudeToXCoordinate(getModel().getLongitude().getDecimalDegrees(), zoomLevel),
                driver.latitudeToYCoordinate(getModel().getLatitude().getDecimalDegrees(), zoomLevel));
        ...
        Point newPoint = new Point((int)((diffedPoint.x) * zoomChange), (int)((diffedPoint.y) * zoomChange));

        Point newViewLocation = new Point(newPoint.x - diff.x, newPoint.y - diff.y);
        ...
        if (newZoomLevel >= driver.getMinZoomLevel() && newZoomLevel <= driver.getMaxZoomLevel()) {
            // getModel().setSuppressAllEvents(true);
            getModel().setLongitude(new Coordinate(driver.xCoordinateToLongitue(newViewLocation.x, newZoomLevel)));
            getModel().setLatitude(new Coordinate(driver.yCoordinateToLatitude(newViewLocation.y, newZoomLevel)));
