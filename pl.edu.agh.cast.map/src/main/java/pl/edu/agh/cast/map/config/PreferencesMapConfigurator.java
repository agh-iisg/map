...
import pl.edu.agh.cast.map.Activator;
...
public class PreferencesMapConfigurator implements IMapConfigurator {
	...
	public boolean discCacheEnabled() {
		return store.getBoolean(Keys.MAP_DISC_CACHE_LOCATION);
	}
	...
	public static class DefualtValuesInitializer extends
		...
		public void initializeDefaultPreferences() {
			...
			store.setDefault(Keys.MAP_MEMORY_CACHE_LIMIT,
					OPERATIONAL_MEMORY / 4);
			...
			if (!mapDirectory.exists() && !mapDirectory.mkdirs()) {
				Logger.getLogger(getClass()).warn(
						"Cannot create directory for map tile"); //$NON-NLS-1$
			}
	...
	public final class Keys {
		/**
		 * <code>MAP_MEMORY_CACHE_ENABLED</code> preference name. This
		 * preference keeps flag that indicates whether memory cache is enabled.
		 */
		public static final String MAP_MEMORY_CACHE_ENABLED = "MAP_MEMORY_CACHE_ENABLED"; //$NON-NLS-1$

		/**
		 * <code>MAP_MEMORY_CACHE_LIMIT</code> preference name. This preference keeps
		 * information how many operational memory should be used for storing
		 * map tiles by memory cache.
		 */
		public static final String MAP_MEMORY_CACHE_LIMIT = "MAP_MEMORY_CACHE_LIMIT"; //$NON-NLS-1$

		/**
		 * <code>MAP_DISC_CACHE_ENABLED</code> preference name. This preference
		 * keeps flag that indicates whether disc cache is enabled.
		 */
		public static final String MAP_DISC_CACHE_ENABLED = "MAP_DISC_CACHE_ENABLED"; //$NON-NLS-1$

		/**
		 * <code>MAP_DISC_CACHE_LOCATION</code> preference name. This preference keeps
		 * information where map tiles for disc cache should be stored.
		 */
		public static final String MAP_DISC_CACHE_LOCATION = "MAP_DISC_CACHE_LOCATION"; //$NON-NLS-1$

		/**
		 * <code>MAP_DISC_CACHE_LIMIT_ENABLED</code> preference name. This
		 * preference keeps information whether storage limit for disc cache is
		 * enabled.
		 */
		public static final String MAP_DISC_CACHE_LIMIT_ENABLED = "MAP_DISC_CACHE_LIMIT_ENABLED"; //$NON-NLS-1$

		/**
		 * <code>MAP_DISC_CACHE_LIMIT</code> preference name. This preference
		 * keeps information how many space should be used for storing map tiles
		 * by disc cache.
		 */
	     ...
	     * used for tile obtaining. <b>CAUTION:</b> collection can contain providers incompatible with selected map driver. To solve this issue
	     * there should be mechanism for filtering providers compatible with specified map driver.
	     */
